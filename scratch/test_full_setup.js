  ...
  try {
    ...
    const finalSql = `
      ...
      WHERE bk.BK_BKBE_STATUS_BEARBEITUNG = 0 
        ...
        AND (bk.BK_BKBE_NUMMER LIKE '%75771%' OR bk.BK_BKBE_NUMMER LIKE '%202675771%')
    `;

    const res = await poolD4.request().query(finalSql);
    const rows = res.recordset;

    // Group steps by OrderId
    const ordersMap = {};
    rows.forEach(r => {
      if (!ordersMap[r.OrderId]) ordersMap[r.OrderId] = [];
      ordersMap[r.OrderId].push(r);
    });

    // Compute realSPKO and color as in server.js
    Object.keys(ordersMap).forEach(oId => {
      ...
      group.forEach((step, idx) => {
        ...
        if (vgRaw === '') {
          if (idx > 0) predPos = group[idx - 1].StepPos;
        } else {
          predPos = vgRaw;
        }
        ...
        console.log(`OrderId ${step.OrderId}, StepId ${step.StepId}, StepPos ${step.StepPos}: ${step.StepDesc?.replace(/\r?\n/g, ' ')} -> realSPKO=${step.realSPKO}, predStepPos=${predStep?.StepPos}, predSPKO=${predStep?.realSPKO}, COLOR=${step.color}`);
      });
    });
