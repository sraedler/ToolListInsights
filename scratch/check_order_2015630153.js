...
async function run() {
  try {
    ...
    const result = await pool.request().query(`
      ...
      WHERE bk.BK_BKBE_NUMMER = '2015630153'
    `);
    
    console.log('Order Details for 2015630153:');
    console.log(result.recordset);
    
    // Check if there are steps associated with it
    const stepsResult = await pool.request().query(`
      SELECT 
        p.ID as StepId,
        p.PSP_BEZEICHNUNG as StepDesc,
        p.PSP_PP_STATUS_PRODUKTION as StepStatus,
        bk.BK_BKBE_STATUS_BEARBEITUNG as OrderStatus
