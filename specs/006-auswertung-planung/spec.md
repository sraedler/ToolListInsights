...
- **FR-009**: System MUST support Night Run Capacity Optimization (Nachtlaufzeit-Optimierung) for Pool Machines. The maximum night capacity ($\text{MaxNightCapacity}$) MUST be calculated as $\text{MaxPiecesPerNight} \times \text{AvgPieceTime}$, where $\text{AvgPieceTime} = \frac{\text{TotalStepProdTime}}{\text{PosQuantity}}$. Day window runtime MUST NOT exceed the standard Day Window Limit (`DayCapacity`). Night shift runtime MUST be maximized up to $\min(\text{MaxNightCapacity}, 24\text{h} - \text{DayShiftPlannedTime})$, such that total combined daily machine load (Day shift + Night run) NEVER exceeds 24 hours (1,440 minutes).
