...
- **FR-013**: System MUST provide an interactive Order/Contract Search input in the control bar for both `Planung Maschinen` and `Planung Maschinen blockiert`. Entering a search term (e.g. `P2026`) MUST hide all non-matching job cards across all columns (e.g. hiding `P2025`). When a search query matches an order, any future steps for that matching order whose start date falls beyond the current visible planning date range (`daysCount`) MUST be automatically included and displayed in the machine's `Überlauf` (Overflow) column so no matching future step is hidden.
